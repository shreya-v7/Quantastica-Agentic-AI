...
def calculate_affordability_from_files(user_ph: str, purchase_price: float, desired_loan_amount: float) -> str:
    ...
    """
    bank_data = load_user_json(user_ph, "fetch_bank_transactions.json")
    ...
    # 3. Calculate estimated EMI (20-year loan at 8.5% p.a.)
    ...
    # 4. Determine affordability (EMI should not exceed 40% of income)
    ...
    if monthly_income > 0:
        ...
        if estimated_emi <= max_permissible_emi:
            ...
            recommendation_text = (
                f"Based on an estimated monthly income of ₹{monthly_income:,.2f}, this loan appears affordable. "
                f"Your existing outstanding debt is ₹{total_outstanding_debt:,.2f}."
            )
        elif estimated_emi <= max_permissible_emi * 1.25:
            affordability = "Manageable with a tight budget"
            recommendation_text = (
                f"This loan would consume a significant portion of your estimated monthly income of ₹{monthly_income:,.2f}, "
                f"making your budget tight. Your existing outstanding debt is ₹{total_outstanding_debt:,.2f}."
            )
        else:
            affordability = "Not Recommended"
            recommendation_text = (
                f"The estimated EMI of ₹{estimated_emi:,.2f} is high compared to your estimated monthly income of ₹{monthly_income:,.2f}. "
